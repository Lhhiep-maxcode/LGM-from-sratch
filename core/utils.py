...
# https://www.notion.so/Field-of-View-Focal-22d7e9ca7f6580028f36f001bcb51cb9?source=copy_link
# This link is for anyone who don't know much about Field Of View, and standardize

# For camera world, * is camera, right in center (things will different with world space)
# Normally, direction of camera is +Z
#                *----------> +X 
#              / |
#             /  |
#           +Z   +Y
...
# With OpenGL, direction of camera is -Z
#                +Y
#                |
#                |
#                *----------> +X 
...
def get_rays(pose, h, w, fovy, opengl=True):
    ...
    # fovy: vertical field of view in degrees (search gg if u dont k)
...
def grid_distortion(images, strength=0.5):
    ...
    # To be simple, imagine num_steps=4 -> [-1.0, 0.33, 0.33, 1.0]
    # And W = 10
    ...
    for b in range(B):
        ...
        x_steps[0] = 0
        x_steps[-1] = W # It can be [0, 2, 7, 10]
        xs = []
        # 0-2 => torch.linspace(-1.0, -0.33, 2) -> [-1.0, -0.33]
        # 2-7 => torch.linspace(-0.33, 0.33, 5) -> [-0.33, -0.16, 0.0, 0.16, 0.33] ....
